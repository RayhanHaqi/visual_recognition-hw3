...
The primary model uses \textbf{ConvNeXt-Base} as the backbone, initialized with ImageNet-22K pretrained weights and fine-tuned at 384$\times$384 resolution\\ (\texttt{convnext\_base.fb\_in22k\_ft\_in1k\_384}). The backbone contains four stages with output channels $[128, 256, 512, 1024]$ at strides $[4, 8, 16, 32]$, totaling 88 million parameters.
...
\begin{itemize}
    \item \textbf{RPN:} Region Proposal Network with custom anchor sizes \\ \texttt{((8,), (16,), (32,), (64,), (128,))} and aspect ratios $(0.5, 1.0, 2.0)$.
...
\textbf{Hypothesis:} Without a validation set ($\texttt{val\_frac}=0.0$), an automated signal is needed to select the best training epoch. The gradient norm (computed via\\ \texttt{clip\_grad\_norm\_}) could serve as a convergence indicator: when the EMA-smoothed gradient norm stabilizes (relative change $<3\%$ over a 10-epoch window), the model has converged.
